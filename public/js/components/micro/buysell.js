...
const positionOverView = (pos, ind) => {
   ...
   return (<div className="trade-description">
    <div className="trn-blk trn-symb">
    <span>Symbol</span>
    <span>{pos.symb}</span></div>
    <div className="trn-blk">
    <span>Quantity</span>
    <span>{pos.qty}</span></div>
    <div className="trn-blk">
    <span>PPU</span>

    <span>{'$'+ (Math.round(pos.point.data[ind] * 100) / 100)}</span></div>
    <div className="trn-blk">
    <span>Expires</span>
    <span>{time}</span></div>
    </div>);
...
export default class CallPut extends React.Component {
   ...
   render() {
      ...
      return (<div className="option-buy-sell reduct"> 
