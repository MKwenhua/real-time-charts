...
export default class Graph extends React.PureComponent {
    ...
    if (this.state.metric !== 'PRICE' && met !== 'PRICE') {
      ...
      let seriesOpts = this.state.activeSymbols.map((dta) => {
        let lineData = this.state.sets[dta].data.map((itm) => {
          return [
            new Date(itm.timestamp).getTime(),
            itm.close
          ];
        });
        return gType === 'step' ? {
  ...
  gotDataM = (details) => {
    console.log('got dataM', details);
    let last = details.results[details.results.length - 1];
    let sets = this.state.sets;
    let isUp = last.open < last.close;
    let newSymb = details.results[0].symbol;
    sets[newSymb] = {
      sym: newSymb,
      data: details.results,
      ...
      <div className='container'>
        <div className='chart-controller'>
          <div className='select-holders'>
            ...
            <select ref='exSymbol' className='symbol-pick' value={this.state.symbol} onChange={this.symbolChange}>
              {exchangeOptions[this.state.exchange]}
            </select>
          </div>
          <div className='select-holders'>
            <strong>Exchange:</strong>
            <select ref='changeName' className='symbol-pick' value={this.state.exchange} onChange={this.exchangeChange}>
        ...
        <div id='chartConfig'>
          <ul className='inline-ul'>
            <li>
              ...
              <select ref='chartType' className='chart-pick' value={this.state.metric} onChange={this.metricChange}>
                <option key={'OHLC'} value='OHLC'>OHLC</option>
                <option key={'PRICE'} value='PRICE'>Stock Price</option>
                <option key={'VOLUME'} value='VOLUME'>Volume</option>
              </select>
            </li>
            <li className='compare' onClick={this.toggleAddSymb}>
        ...
        <div id='add-symbol' className={this.state.addSymb ? 'open' : 'close'}>
          ...
          <select ref='addSymbol' className='add-in'>

            {exchangeOptions[this.state.exchangeAd]}
          </select>
          <span className='span-label'>exchange:</span>
          <select ref='changeEXAd' className='add-in' value={this.state.exchange} onChange={this.exchangeChangeAdd}>
            <option key='NASDAQ-add' value='NASDAQ'>NASDAQ</option>
            <option key='NYSE-add' value='NYSE'>NYSE</option>
          </select>
          <span className='add-symbol-submit' onClick={this.symbolAdd}>
            ADD
          </span>
        </div>
        <div onClick={this.graphType} className={cShow ? 'chart-controller chart-set' : 'chart-controller chart-set hide-elm'}>
