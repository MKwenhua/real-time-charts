...
export default class CallPut extends React.PureComponent {
   ...
   toggleMenuBid = () => {
      const pos = this.state.pos ? this.state.pos : {
         qty: 1,
   ...
   setPosTime = (tm) => {
      const pos = this.state.pos ? Object.assign({}, this.state.pos) : {
         qty: 1,
         time: this.state.tmSet,
         point: {
            data: [2, 3, 2, 2]
   ...
   posSet = (bidOrAsk) => {
      const {pos, posNum} = Object.assign({}, this.state);
      const d = new Date();
   ...
   render() {
      ...
      const askOverview = this.positionOverView(1);
      const bidOverview = this.positionOverView(2);
      return (
