...
function liveStream(room, indexx, lastVal){
  ...
  if(userNrooms[room].length > 0){
   io.sockets.in(room).emit(room +'_Feed',{ symb: room , data: tickData.eur_usd_m1[indexx] ,  lastVal: lastVal, min: theMin}); 
 	}
  let ind = (indexx + 1) < tickData.len ? indexx + 1 : 0;
	setTimeout(function(){  liveStream(room,  ind, yVal); }, 100);
}
